...
class AdvancedPerformanceMonitor {
  ...
  void _onFrame(Duration timestamp) {
    ...
    // REMOVED: Recursive frame callback that was causing performance issues
    // SchedulerBinding.instance.addPersistentFrameCallback(_onFrame);
  }
  ...
  void endOperation(String operationName, {Map<String, dynamic>? metadata}) {
    ...
    if (stopwatch != null) {
      ...
      if (kDebugMode) {
        debugPrint('Completed operation: $operationName (${stopwatch.elapsedMilliseconds}ms)');
      }
      
      // Alert for slow operations
      if (stopwatch.elapsedMilliseconds > 1000) {
        if (kDebugMode) {
          debugPrint('Slow operation detected: $operationName took ${stopwatch.elapsedMilliseconds}ms');
        }
      }
    }
  }
  ...
  Future<void> _recordMemorySnapshot() async {
    try {
      ...
      if (kDebugMode) {
        // TEMPORARILY DISABLED - Excessive logging causing performance issues
  ...
  void _generatePerformanceReport() {
    ...
    if (report.averageFPS < 45) {
      // Low FPS warning disabled temporarily
    }
    
    if (report.memoryUsage > 200) {
      // High memory warning kept for critical issues
      if (kDebugMode) {
